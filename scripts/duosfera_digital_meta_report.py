#!/usr/bin/env python3
"""
Duosfera Digital - Relatorio Semanal Meta Ads
Conta: act_767452304695831 | Moeda: BRL
...
print("Buscando dados Meta Ads Duosfera Digital...")
...
budget_est=1000  # placeholder — atualizar com verba real do cliente
dias_mes=today.day
...
L=[]; A=L.append
A(f"# Relatorio Meta Ads - Duosfera Digital")
...
A(f"Projecao mensal: R${curr['spend']/7*30:.0f} | Budget: R${budget_est} (placeholder)")
...
A(f"Meta Ads Duosfera Digital - {today.strftime('%d/%m/%Y')}")
...
report="\n".join(L)
filepath=f"reports/DUOSFERA/duosfera-digital/{mes}/meta-duosfera-digital-report-{today.strftime('%Y-%m-%d')}.md"
...
payload={"message":f"feat: relatorio Meta Ads Duosfera Digital {today.strftime('%Y-%m-%d')}","content":base64.b64encode(report.encode()).decode()}
