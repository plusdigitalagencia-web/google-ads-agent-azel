#!/usr/bin/env python3
"""
Alcala Homes - Relatorio Semanal Meta Ads (7 Modulos)
SOP: data-know/SOP-analise-meta-ads.md
Conta: act_574789065003576 | Moeda: EUR | Imoveis de luxo Madrid
Roda toda segunda via GitHub Actions as 07:10 BRT
Token: ALCALA_META_TOKEN | App Meta: 687818438366946
"""
import os, urllib.request, urllib.error, urllib.parse
import json, datetime, base64, time

TOKEN        = os.environ["ALCALA_META_TOKEN"]
GITHUB_TOKEN = os.environ["GH_PAT"]
ACCOUNT      = "act_574789065003576"
BASE         = "https://graph.facebook.com/v25.0"
GITHUB_REPO  = "plusdigitalagencia-web/google-ads-agent-azel"
CLIENT_NAME  = "Alcala Homes"
CURRENCY     = "€"
...
for r in sorted(camp_curr, key=lambda x: float(x.get("spend", 0)), reverse=True):
    ...
    if l == 0:
        ...
        if ctr < 0.8:
            A("- Causa raiz: CTR baixo -> criativo nao gera interesse no publico de compradores de luxo")
            A("- Acao: testar angulo exclusividade (off-market, acesso privado) ou visuais de alto impacto")
        else:
            A("- Causa raiz: CTR adequado mas sem conversao -> formulario com atrito ou publico desqualificado")
            A("- Acao: revisar formulario nativo e segmentacao — garantir que chega em investidores e compradores premium")
    else:
        c = sp/l
        if c > avg_cpl * 1.8:
            pct_above = int((c/avg_cpl-1)*100)
            A(f"- 🔴 CPL {cpl_str(sp,l)} = {pct_above}% acima da media ({cpl_str(curr['spend'],curr['leads'])})")
        elif c > avg_cpl * 1.2:
        ...
        if curr["avg_cpm"] > 0 and cpm > curr["avg_cpm"] * 1.5:
            ...
            causes.append(f"CPM {cur(cpm)} ({pct_cpm}% acima da media) -> audience premium competitiva")
        if ctr < 0.8:
...
for r in ad_curr:
    ...
    if prev_r:
        ...
        if d_cpm is not None and d_cpm > 40:
            anomalies.append(f"| CPM SUBIU | {aname[:35]} | {cur(prev_cpm)} -> {cur(cpm)} ({d_cpm:+.0f}%) | Audience premium mais competitiva |")
...
if winners:
    ...
    A(f"- Testar variacao: off-market angle + visual de imovel premium Madrid")
    total_leads = int(curr["leads"])
    if total_leads < 100:
        A(f"- Continuar acumulando leads ({total_leads}/100) para ativar lookalike de compradores de luxo")
    else:
        A(f"- {total_leads} leads acumulados - criar lookalike baseado nos convertidos")
else:
    A("- Nenhum criativo com leads no periodo - testar angulo off-market e visuais de imoveis Salamanca/Retiro")
A("")
if losers:
...
if adset_curr:
    ...
    for cn, asets in camp_adsets.items():
        if len(asets) > 2:
            A(f"- Campanha {cn} com {len(asets)} ad sets simultaneos - verificar sobreposicao de publico premium")
    A("")
else:
...
for camp in camp_curr:
    ...
    if ads_nl:
        ...
        A(f"- Frequencia em {freq:.2f} - risco de saturacao do publico premium")
    else:
...
filepath = f"reports/data-know/alcala-homes/{mes_pasta}/meta-report-{today.strftime('%Y-%m-%d')}.md"
report   = "\n".join(L)
...
save_to_github(filepath, report, f"feat: relatorio Meta Ads Alcala Homes {today.strftime('%Y-%m-%d')}")
