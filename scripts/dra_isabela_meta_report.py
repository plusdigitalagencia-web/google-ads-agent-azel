#!/usr/bin/env python3
"""
Dra. Isabela Gomes - Relatorio Semanal Meta Ads
Conta: act_1181839902697616 | Moeda: BRL
Roda toda segunda via GitHub Actions as 07:00 BRT
IMPORTANTE: nao usar o termo "Dermatologista" — pos concluida, titulo nao emitido ainda
"""
...
budget_est=600  # R$ 600/mes — atualizar conforme ajuste de verba
dias_mes=today.day
...
L=[]; A=L.append
A(f"# Relatorio Meta Ads - Dra. Isabela Gomes")
...
report="\n".join(L)
filepath=f"reports/DUOSFERA/dra-isabela/{mes}/meta-dra-isabela-report-{today.strftime('%Y-%m-%d')}.md"
...
payload={"message":f"feat: relatorio Meta Ads Dra Isabela {today.strftime('%Y-%m-%d')}","content":base64.b64encode(report.encode()).decode()}
