#!/usr/bin/env python3
"""
Onvoix - Relatorio Semanal Meta Ads
Conta: act_1297650828540575 | Moeda: USD | BM: Unified Boost Holdings Ltd.
...
TOKEN        = os.environ["ONVOIX_META_TOKEN"]
GITHUB_TOKEN = os.environ["GH_PAT"]
ACCOUNT      = "act_1297650828540575"
...
def cpl_str(spend, l):
    return f"US${spend/l:.2f}" if l > 0 else "---"
...
print("Buscando dados Meta Ads Onvoix...")
...
A("|---|---|---|---|")
A(f"| Gasto | US${curr['spend']:.2f} | US${prev['spend']:.2f} | {pct(curr['spend'],prev['spend'])} |")
...
A(f"| CPM medio | US${curr['avg_cpm']:.2f} | US${prev['avg_cpm']:.2f} | {pct(curr['avg_cpm'],prev['avg_cpm'])} |")
...
for r in sorted(camp_curr, key=lambda x:float(x.get("spend",0)),reverse=True):
    ...
    A(f"| {r.get('campaign_name','---')} | US${sp:.2f} | {float(r.get('ctr',0)):.2f}% | US${float(r.get('cpm',0)):.2f} | {freq:.2f}{fw} | {int(l)} | {cpl_str(sp,l)} | {st} |")
A("")
...
for cname,ads in camps_map.items():
    ...
    for r in with_l+no_l:
        ...
        A(f"| {r.get('ad_name','---')[:38]}{star} | US${sp:.2f} | {float(r.get('ctr',0)):.2f}% | US${float(r.get('cpm',0)):.2f} | {freq:.2f}{fw} | {int(l)} | {cpl_str(sp,l)} | {st} |")
    A("")

A("---"); A("")
A("## Modulo 4 - Pacing Monitor"); A("")
A(f"Dia {dias_mes} de {last_day} ({pct_mes:.0f}% do mes). Gasto semanal: US${curr['spend']:.2f}")
A(f"Projecao mensal: US${curr['spend']/7*30:.0f} | Budget estimado: US${budget_est}")
...
for r in ad_curr:
    ...
    if sp>30 and l==0:
        acts.append(f"| CRITICO | Pausar '{r.get('ad_name','?')[:28]}' (US${sp:.0f}, 0 leads) | Liberar budget | Imediato |")
for r in ad_curr:
    l=leads(r);sp=float(r.get("spend",0))
    if l>0:
        c=sp/l
        if c>avg_cpl*2 and sp>50:
            acts.append(f"| ATENCAO | Revisar '{r.get('ad_name','?')[:26]}' (CPL US${c:.0f} vs media US${avg_cpl:.0f}) | Reduzir CPL | {nm.strftime('%d/%m')} |")
for r in ad_curr:
...
A(f"Meta Ads Onvoix - {today.strftime('%d/%m/%Y')}")
A(f"Gasto: US${curr['spend']:.2f} | Leads: {int(curr['leads'])} | CPL: {cpl_str(curr['spend'],curr['leads'])} ({pct(curr['cpl_val'],prev['cpl_val']) if curr['leads'] and prev['leads'] else '---'} vs semana anterior)")
A("")
...
for camp in camp_curr:
    ...
    if best:
        ...
        A(f"- Criativo \"{best.get('ad_name','?')[:40]}\" - CTR {float(best.get('ctr',0)):.2f}% | CPL US${bc:.2f}")
    else:
    ...
    if ads_nl:
        for bad in ads_nl[:2]:
            A(f"- \"{bad.get('ad_name','?')[:40]}\" - US${float(bad.get('spend',0)):.2f} gastos com 0 leads")
    ...
    if best:
        bl=leads(best);bc=float(best.get("spend",0))/bl
        A(f"- {best.get('ad_name','?')[:40]} - CTR {float(best.get('ctr',0)):.2f}% | CPL US${bc:.2f}")
    else: A("- A definir")
    A("")
...
report="\n".join(L)
filepath=f"reports/data-know/onvoix/meta-report-{today.strftime('%Y-%m-%d')}.md"
...
payload={"message":f"feat: relatorio Meta Ads Onvoix {today.strftime('%Y-%m-%d')}","content":base64.b64encode(report.encode()).decode()}
