#!/usr/bin/env python3
"""
Onvoix - Relatorio Semanal Meta Ads (7 Modulos)
SOP: data-know/SOP-analise-meta-ads.md
Conta: act_1297650828540575 | Moeda: USD | Budget: USD500/mes
Roda toda segunda via GitHub Actions as 07:00 BRT
Token: LEBLON_META_TOKEN (tem acesso a esta conta) | Expira: 05/08/2026
"""
...
for r in sorted(camp_curr, key=lambda x: float(x.get("spend", 0)), reverse=True):
    ...
    if l == 0:
        ...
        if ctr < 0.8:
            A("- Causa raiz: CTR muito baixo -> criativo nao gera interesse no publico")
            A("- Acao: testar novo criativo ou revisar targeting")
        else:
...
if pacing_st == "NO RITMO":
    ...
    diff_int = int(diff_p)
    A(f"- Status: {pacing_icon} ACELERADO (+{diff_int}% acima do esperado) - risco de esgotar budget antes do fim do mes")
else:
    diff_int = int(diff_p)
    A(f"- Status: {pacing_icon} LENTO ({diff_int}% abaixo do esperado) - pode faltar entrega no fim do mes")
A(""); A("---"); A("")
...
if winners:
    ...
    A(f"- Testar variacao do hook de {bn} com angulo diferente, mesmo publico")
    total_leads = int(curr["leads"])
    if total_leads < 100:
        A(f"- Continuar acumulando leads ({total_leads}/100) para ativar lookalike")
    else:
        A(f"- {total_leads} leads acumulados - criar lookalike baseado nos convertidos")
else:
    A("- Nenhum criativo com leads no periodo - revisar criativos e targeting")
A("")
if losers:
...
if adset_curr:
    ...
    for cn, asets in camp_adsets.items():
        if len(asets) > 2:
            A(f"- Campanha {cn} com {len(asets)} ad sets simultaneos - verificar sobreposicao de publico")
    A("")
else:
