#!/usr/bin/env python3
"""
Leblon Real Estate - Relatorio Semanal Meta Ads
Conta: act_1187011709535726 | Moeda: EUR
...
L=[]; A=L.append
A(f"# Relatorio Meta Ads - Leblon Real Estate")
...
for r in ad_curr:
    ...
    if sp>30 and l==0:
        acts.append(f"| CRITICO | Pausar '{r.get('ad_name','?')[:28]}' (EUR{sp:.0f}, 0 leads) | Liberar budget | Imediato |")
for r in ad_curr:
...
A(f"Meta Ads Leblon Real Estate - {today.strftime('%d/%m/%Y')}")
