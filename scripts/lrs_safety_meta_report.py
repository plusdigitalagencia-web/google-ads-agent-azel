...
CURRENCY     = "R$"
BUDGET_EST   = 1800  # BRL/mes (R$60/dia x 30)
...
for r in sorted(camp_curr, key=lambda x: float(x.get("spend", 0)), reverse=True):
    ...
    if l == 0:
        ...
        if ctr < 0.8:
            A("- Causa raiz: CTR baixo -> anuncio nao gera interesse nos decisores industriais")
            A("- Acao: testar angulo de dor (multa MTE, acidente, documentacao vencida)")
        else:
            A("- Causa raiz: CTR adequado mas sem conversao -> verificar formulario/landing page ou qualificacao do publico")
            A("- Acao: revisar CTA e segmentacao — garantir que chega em diretores e gerentes industriais")
    else:
        c = sp/l
        if c > avg_cpl * 1.8:
...
if winners:
    ...
    A(f"- Testar variacao do hook de {bn} com angulo diferente (ex: multa vs. acidente vs. documentacao)")
    total_leads = int(curr["leads"])
    if total_leads < 100:
        A(f"- Continuar acumulando leads ({total_leads}/100) para ativar lookalike de decisores industriais")
    else:
        A(f"- {total_leads} leads acumulados - criar lookalike baseado nos convertidos")
else:
    A("- Nenhum criativo com leads no periodo - revisar angulos de copy e targeting para decisores industriais SP")
A("")
if losers:
...
if adset_curr:
    ...
    for cn, asets in camp_adsets.items():
        if len(asets) > 2:
            A(f"- Campanha {cn} com {len(asets)} ad sets simultaneos - verificar sobreposicao de publico industrial SP")
    A("")
else:
...
filepath = f"reports/data-know/lrs-safety/{mes_pasta}/meta-report-{today.strftime('%Y-%m-%d')}.md"
...
save_to_github(filepath, report, f"feat: relatorio Meta Ads LRS Safety {today.strftime('%Y-%m-%d')}")
