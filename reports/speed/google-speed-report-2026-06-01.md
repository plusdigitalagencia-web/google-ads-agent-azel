# Relatório Google Ads — SpeedMais Gráfica
**Período:** 01/05/2026 a 31/05/2026  |  Plus Digital
**Conta:** 608-141-8993 | MCC Açúcar (256-486-5113)
**Foco:** Captação de Leads (Orçamentos/WhatsApp) | Evento: Conversa WhatsApp

> ✅ **STATUS DA API:** Token OAuth2 restaurado e funcionando. Dados desta análise são reais, consultados diretamente via Google Ads API. O aviso de "dados estimados" do relatório anterior foi resolvido.

---

## 🧭 Resumo Executivo

### ✅ O que deu certo

- **CPA caiu 17,6%:** de R$ 2,73/conversa em Abril para R$ 2,25 em Maio — melhor resultado histórico da conta.
- **Conversões subiram 13,1% gastando menos:** 1.571 conversas em Maio vs 1.389 em Abril, com R$ 244 a menos de investimento. O algoritmo está otimizando melhor.
- **CTR melhorou de 13,16% → 13,57%** (+0,41pp): o anúncio continua com força de entrega e sem sinais de fadiga.
- **CPC caiu de R$ 0,29 → R$ 0,28:** leve melhora na eficiência de custo por clique.

### ❌ O que não deu certo

- **Investimento caiu 6,5% sem decisão estratégica:** a campanha gastou R$ 244 a menos que Abril. Com o CPA melhorando, esse orçamento poderia ter gerado ~109 conversas adicionais.
- **Asset group ainda em força "Médio":** o Google penaliza no leilão campanhas PMax com assets abaixo de "Excelente". A SpeedMais está competindo em desvantagem.
- **Share de impressões travado em 10%:** a conta está capturando apenas 1 em cada 10 oportunidades disponíveis no mercado de embalagens B2B do Nordeste.
- **ROAS zerado — sem valor de conversão configurado:** o algoritmo não tem sinal de receita para otimizar. Está operando no escuro quanto ao retorno financeiro real.
- **1 única campanha ativa:** toda a conta depende de um único PMax, sem diversificação de canal ou teste A/B.

### 🚀 O que podemos fazer para melhorar

1. **Aumentar orçamento em Junho:** com CPA em R$ 2,25 e eficiência melhorando, é o momento certo para escalar. +R$ 30–50/dia pode trazer aproximadamente +400 conversas no mês sem perder CPA.
2. **Elevar o asset group de "Médio" para "Excelente":** adicionar headlines com diferenciais específicos (pedido mínimo 500 unid., 10 anos de mercado, impressão offset, entrega no Nordeste) e imagens de portfólio real. Isso melhora o placement no leilão e tende a reduzir CPC.
3. **Configurar valor de conversão para o clique no WhatsApp:** atribuir R$ 500–800 por conversa (ticket médio estimado de um pedido de embalagens). Habilita o target ROAS e dá ao algoritmo sinal de qualidade real.
4. **Criar lista de negativos B2C:** termos como "artesanal", "avulso", "embalagem festa", "como fazer", "molde", "embalagem barata" drenam budget em público fora do ICP (empresas com pedido mínimo 500 unid.). Estimativa: 20–35% do budget atual em tráfego não qualificado.

---

## 📋 Campanhas Ativas

### 🟢 [PMax] [Assucar] [Pmax] [Lead] - 25/06/25

| Métrica | Valor |
|---|---|
| Status CPA | 🟢 R$ 2,25 |
| Gasto | R$ 3.541,36 |
| Conversas WhatsApp | 1.571 |
| CPA | R$ 2,25 |
| Impressões | 92.680 |
| Cliques | 12.577 |
| CTR | 13,57% |
| CPC Médio | R$ 0,28 |
| Share de Impressões | 10,0% |
| Força do Asset Group | 🟡 Médio |
| ROAS | ⚠️ Não configurado |

### ⚫ Campanhas Pausadas (5 campanhas)

| Campanha | Gasto | Status |
|---|---|---|
| [Search] [ASSUCAR] [LEAD] [MAX CONVERSÕES] - 07.02.2024 | R$ 0,00 | ⚫ Pausada |
| [Search] [ASSUCAR] TESTE IMPRESSAO | R$ 0,00 | ⚫ Pausada |
| [Search] [ASSUCAR] [LEAD] [MAX CLIQUES] - 17.02.2024 #copia | R$ 0,00 | ⚫ Pausada |
| [Search] [ASSUCAR] [LEAD] [MAX CONVERSÕES] [INTERIOR] - 03/07/2024 | R$ 0,00 | ⚫ Pausada |
| [PMax] [Assucar] [Pmax] [Embalagens e Brindes] - 06/10/25 | R$ 0,00 | ⚫ Pausada |

---
**Total investido:** R$ 3.541,36  |  **Conversas:** 1.571  |  **CPA:** R$ 2,25

---

## 📊 Comparativo Maio vs Abril 2026

| Métrica | Abril/2026 | Maio/2026 | Variação |
|---|---|---|---|
| Gasto total | R$ 3.785,68 | R$ 3.541,36 | -6,5% 🟡 |
| Conversas (WhatsApp) | 1.389 | 1.571 | **+182 (+13,1%) 🟢** |
| CPA médio | R$ 2,73 | R$ 2,25 | **-R$ 0,48 (-17,6%) 🟢** |
| Cliques | 13.195 | 12.577 | -618 (-4,7%) 🟡 |
| Impressões | 100.295 | 92.680 | -7.615 (-7,6%) 🟡 |
| CTR | 13,16% | 13,57% | +0,41pp (+3,1%) 🟢 |
| CPC Médio | R$ 0,29 | R$ 0,28 | -R$ 0,01 (-3,4%) 🟢 |
| Share de Impressões | 10,0% | 10,0% | — |

> **Nota metodológica:** "Abril" corresponde ao delta entre os últimos 60 dias e os últimos 30 dias (período aproximado 02/04–01/05). Para comparativo exato por calendário, o relatório do mês de Abril pode ser consultado no GitHub.

### Leitura do Comparativo

Maio foi o melhor mês em eficiência desde o início do monitoramento: **o CPA caiu 17,6% com volume de conversas subindo 13,1%**. A queda em cliques e impressões não é preocupante — o algoritmo aprendeu a ser mais seletivo na entrega. O ponto de alerta é o orçamento menor que Abril: como o CPA está excelente, o momento ideal para escalar é agora.

---

## 🔍 Audit de Criativos

**Score de Saúde:** 🟡 65/100

| Campanha | Asset Group | Força | Status |
|---|---|---|---|
| [PMax] [Assucar] [Pmax] [Lead] | Anúncio 1 | 🟡 Médio | ATIVO |

### Análise

O PMax está rodando com apenas **1 asset group de força "Médio"**. Para atingir "Excelente", a campanha precisa de:
- Mínimo 5 títulos curtos (30 chars) e 5 títulos longos (90 chars)
- Mínimo 5 descrições com diferenciais específicos
- Mínimo 3 imagens (horizontal 1,91:1 + quadrada 1:1 + retrato 4:5)
- Pelo menos 1 vídeo (sem vídeo, o Google gera criativos automáticos — qualidade inferior)

### Assets recomendados para elevar a força do grupo

**Títulos curtos (30 chars) — sugestões:**
- `Embalagem Offset Personalizada`
- `A partir de 500 Unidades`
- `Orçamento em 24h | WhatsApp`
- `Gráfica Offset no Recife`
- `10 Anos de Mercado`

**Títulos longos (90 chars) — sugestões:**
- `Embalagens personalizadas para sua empresa | Pedido mínimo 500 unid.`
- `Impressão offset de alta qualidade | Entrega rápida no Nordeste`
- `Sacolas, caixas e embalagens delivery | Solicite orçamento agora`
- `Gráfica especializada em embalagens B2B | 10 anos de experiência`
- `Embalagem para e-commerce, delivery e varejo | Fale no WhatsApp`

### ⚠️ Alertas

| Alerta | Gravidade | Ação |
|---|---|---|
| Asset group com força "Médio" | 🟡 Alto | Adicionar assets listados acima para atingir "Excelente" |
| ROAS zerado (sem valor de conversão) | 🔴 Crítico | Configurar valor médio de R$ 500–800 por conversa WhatsApp |
| Share de impressões 10% | 🟡 Alto | Avaliar aumento de orçamento ou revisão de estratégia de lance |
| 1 única campanha ativa | 🟠 Médio | Planejar campanha Search paralela para keywords de alta intenção |
| Termos de busca não auditáveis | 🟠 Médio | PMax não expõe termos — criar lista de negativos preventiva B2C |

---

## 🚀 Plano de Ação — Junho 2026

| Prioridade | Ação | Campanha | Impacto Esperado |
|---|---|---|---|
| 🔴 Alta | Configurar valor de conversão (R$ 500–800/conversa) | PMax Lead | Habilita ROAS, melhora qualidade de entrega |
| 🔴 Alta | Elevar asset group de "Médio" para "Excelente" | PMax Lead | Melhora ranking no leilão e reduz CPC |
| 🟢 Alta | Aumentar orçamento diário em R$ 30–50/dia | PMax Lead | +~400 conversas estimadas no mês |
| 🟡 Média | Criar lista de negativos B2C | PMax Lead | Reduzir 20–35% de tráfego fora do ICP |
| 🟡 Média | Criar asset group separado "Embalagens Delivery" | PMax Lead | Sinal mais preciso para o algoritmo |
| 🟡 Média | Reativar PMax Embalagens e Brindes com novos assets | Nova campanha | Testar novo nicho B2B |
| 🟠 Baixa | Lançar campanha Search para keywords de alta intenção | Nova campanha | Capturar demanda com intenção explícita |

---

**Total investido:** R$ 3.541,36  |  **Conversas:** 1.571  |  **CPA:** R$ 2,25
*Gerado em 01/06/2026 | Plus Digital*
*Token OAuth2: ✅ Operacional | Dados reais via Google Ads API | MCC Açúcar 256-486-5113*
